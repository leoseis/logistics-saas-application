...
export type ApiOrder = { id:string; reference:string; vendor:string; vendor_name:string; rider:string|null; rider_name:string|null; rider_phone:string|null; rider_status:ApiRiderStatus|null; pickup_address:string; delivery_address:string; recipient_name:string; recipient_phone:string; status:ApiOrderStatus; weight_kg:string|null; price_per_kg:string|null; delivery_fee:string; package_photo:string|null; pickup_code?:string; created_at:string; updated_at:string }
...
export function createOrder(payload:OrderCreateInput,packagePhoto?:File|null){const body=new FormData();Object.entries(payload).forEach(([key,value])=>{if(value!==null)body.append(key,String(value))});if(packagePhoto)body.append('package_photo',packagePhoto);return request<ApiOrder>('/orders/',undefined,{method:'POST',body}).then(toOrder)}
export function verifyPickup(orderId:string,pickupCode:string){return request<ApiOrder>(`/orders/${orderId}/verify-pickup/`,undefined,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({pickup_code:pickupCode})}).then(toOrder)}
